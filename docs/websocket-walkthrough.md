...
| `match:round_result` (possession resolver) | `{matchId, qIndex, questionKind, reveal: {kind, ...}, players, phaseKind, phaseRound, shooterSeat, attackerSeat, deltas: {possessionDelta, penaltyOutcome, goalScoredBySeat, speedStreakBoostedSeat?}}` | Round complete (ranked / friendly possession). Narrow on `reveal.kind` for the correct answer — there is **no top-level `correctIndex`**. In a penalty duel, `penaltyOutcome` is `goal` iff the shooter is correct and the keeper wrong, or the shooter out-scores the keeper; equal points is `saved` |
| `match:round_result` (party quiz) | `{matchId, qIndex, questionKind: 'multipleChoice', correctIndex, reveal: {kind: 'multipleChoice', correctIndex}, players, rankingOrder, phaseKind: 'normal', phaseRound, shooterSeat: null, attackerSeat: null}` | Round complete (friendly party quiz). MCQ only, so `correctIndex` is ALSO present at the top level (mirrors `reveal.correctIndex`); `rankingOrder` is the live standings; no `deltas` |
