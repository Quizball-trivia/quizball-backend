...
// Turkish sibling of import-spanish-content.ts. Default --target=staging.
// Dry-run: npx tsx scripts/import-turkish-content.ts
// Apply:   npx tsx scripts/import-turkish-content.ts --apply
// Production requires --target=production --confirm-production=lfbwhxvwubzeqkztghok
// Campaign-quiz tr_* columns arrive with the SEO branch; scalar targets whose column
// is missing are reported as missingColumns and skipped, so the import can be re-run later.
...
const SCALAR_TARGETS: Record<string, { key: string; columns: ReadonlySet<string> }> = {
  campaign_quizzes: {
    key: 'slug',
    columns: new Set([
      'tr_title', 'tr_h1', 'tr_seo_title', 'tr_meta_description',
      'tr_breadcrumb_label', 'tr_lede', 'tr_about_heading', 'tr_about_blocks',
      'tr_hero_image_alt', 'tr_og_image_alt', 'tr_score_cta', 'tr_footer_banner_text',
      'tr_footer_button_label',
    ]),
  },
...
function applyLocaleValue(root: unknown, item: PlanItem, overwrite: boolean): 'changed' | 'existing' | 'conflict' {
  ...
  if (current && typeof current === 'object' && !Array.isArray(current)) {
    ...
    if (typeof field.tr === 'string' && field.tr.trim() && !overwrite) {
      return normalized(field.tr) === normalized(item.translated) ? 'existing' : 'conflict';
    }
    field.tr = item.translated;
    return 'changed';
  }

  if (typeof current === 'string' && normalized(current) === normalized(item.source)) {
    const replacement = { en: current, tr: item.translated };
...
async function planScalarChanges(
  ...
  const stats = { changed: 0, existing: 0, conflicts: 0, missingRows: 0, missingColumns: 0 };
  const changes: Array<{ table: string; keyColumn: string; rowKey: string; column: string; value: unknown }> = [];
...
async function planLocaleRows(
  ...
  for (const batch of chunks(familyIds)) {
    const existingRows = await sql.unsafe<{ card_family_id: string }[]>(
      `SELECT card_family_id::text AS card_family_id
       ...
       WHERE card_family_id::text = ANY($1::text[]) AND locale = 'tr'`,
      [batch],
    );
  ...
  const translatedRows = await sql.unsafe<{ review_notes: string | null }[]>(
    `SELECT review_notes
     FROM public.player_clue_cards
     WHERE locale = 'tr' AND review_notes LIKE 'Turkish translation of %'`,
  );
  for (const row of translatedRows) {
    const match = row.review_notes?.match(/^Turkish translation of ([0-9a-f-]{36});/i);
  ...
  for (const [rowKey, operations] of rowGroups) {
    ...
    inserts.push({
      ...
      review_notes: `Turkish translation of ${rowKey}; semantic QA passed; Turkish catalog import`,
...
async function main() {
  ...
  const databaseUrl = process.env.TURKISH_IMPORT_DATABASE_URL ?? process.env.DATABASE_URL;
  if (!databaseUrl) throw new Error('Set TURKISH_IMPORT_DATABASE_URL or DATABASE_URL');
  ...
  try {
    ...
    const planPath = path.resolve(argValue('plan') ?? '../quizball-question-agents/output/turkish-translation/import-plan.json');
