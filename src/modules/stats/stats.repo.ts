  ...
  async listRecentMatchesForUser(userId: string, limit: number): Promise<RecentMatchRow[]> {
    return sql<RecentMatchRow[]>`
      SELECT
        ...
        COALESCE(opp.is_ai, false) AND opp.ai_kind IS DISTINCT FROM 'persistent' AS opponent_is_ai,
        CASE
          WHEN opp.is_deleted = true OR opp.deleted_at IS NOT NULL OR opp.pending_deletion_at IS NOT NULL THEN NULL
          ELSE opp_ranked.rp
        END AS opponent_rp,
      ...
      LEFT JOIN LATERAL (
        SELECT json_agg(
          json_build_object(
            ...
            'isAi', COALESCE(ou.is_ai, false) AND ou.ai_kind IS DISTINCT FROM 'persistent',
