...
function getNextActorId(members: Array<{ user_id: string }>, bans: Array<{ user_id: string }>, hostUserId: string): string {
...
export function registerDraftHandlers(io: QuizballServer, socket: QuizballSocket): void {
  socket.on('draft:ban', async (payload) => {
    ...
    if (!categoryIds.has(parsed.data.categoryId)) {
      logger.warn({ lobbyId, categoryId: parsed.data.categoryId }, 'Category not in lobby pool');
      return;
    }
    ...
    try {
      await lobbiesRepo.insertLobbyCategoryBan(lobbyId, socket.data.user.id, parsed.data.categoryId);
    } catch (error) {
      logger.warn({ error, lobbyId }, 'Failed to insert lobby ban');
      return;
    }
    logger.info(
      { lobbyId, userId: socket.data.user.id, categoryId: parsed.data.categoryId },
      'Draft ban applied'
    );
